(function (O) {
  ...
  function drawGameBoard(board, player, moves) {
    ...
    moves.forEach(function (m) {
      if (!m.isPassingMove)
        attackable[O.ix(m.x, m.y)] = true; //<td class="cell black attackable" id="cell_3_2"><span class="disc"></span></td>//
    });
  ...
  function setUpUIToChooseMove(gameTree) {
    $('#message').text('Choose your move.');                        //The forEach() method calls a function once for each element in an array, in order//
    gameTree.moves.forEach(function (m, i) {
      if (m.isPassingMove) {
        $('#console').append(                                       //The append() method inserts specified content at the end of the selected elements.
